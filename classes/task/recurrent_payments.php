...
class recurrent_payments extends \core\task\scheduled_task {
    ...
    public function get_name() {
        return get_string('pluginname', 'paygw_yoomoney');
    }
    /**
     * Execute.
     */
    public function execute() {
        ...
        $yoomoneytx = $DB->get_records_sql('SELECT * FROM {paygw_yoomoney} WHERE (success=1 OR success=3) ' .
                  'AND recurrent>=? AND recurrent<?', [ $stime, $ctime ]);

        foreach ($yoomoneytx as $data) {
            // Get payment data.
        ...
        foreach ($yoomoneytx as $data) {
            ...
            // Make payment.
            $location = 'https://api.yoomoney.ru/v3/payments';
            $options = [
            ...
            if (
                ...
                $data->recurrent = 0;
                $DB->update_record('paygw_yoomoney', $data);
                // Notify user.
                notifications::notify(
                    $userid,
                    $cost,
